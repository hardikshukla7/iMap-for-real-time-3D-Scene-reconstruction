...
class IMAP(nn.Module):
    def __init__(self):
        ...
        self.B = nn.Linear(3,93, bias=False).to(device)
...
class Camera():
    def __init__(self, rgb, depth, px,py,pz,rx,ry,rz,a=0.0, b=0.0,fx=525.0, fy=525.0, cx=319.5, cy=239.5):
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.params = torch.tensor([rx,ry,rz,px,py,pz,a,b]).detach().requires_grad_(True)
        # Camera Calibrations
        self.fx = fx
        self.fy = fy
        self.cx = cx
        ...
        self.K = torch.from_numpy(np.array([
            ...
            [0.0, fy, cy],
            [0.0, 0.0, 1.0],
            ]).astype(np.float32)).to(device).requires_grad_(False)
        ...
        # Images cache
        self.rgb = torch.from_numpy((rgb).astype(np.float32))*(1.0/256)
        self.depth = torch.from_numpy(depth.astype(np.float32))*self.factor
        # Lighting parameter
        self.exp_a = torch.FloatTensor(1)
        self.R = torch.FloatTensor(3,3).fill_(0)
        self.T = torch.FloatTensor(3,3).fill_(0)
        self.Li = torch.FloatTensor(64).fill_(1.0/64)
    ...
    def setImages(self, rgb, depth):
        ...
        self.rgb = torch.from_numpy((rgb).astype(np.float32)).to(device)*(1.0/256)
        self.depth = torch.from_numpy(depth.astype(np.float32)).to(device)*self.factor
    # Calc Transform from camera parameters
    def update_transform(self):
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

        i = torch.FloatTensor(3,3).fill_(0)
        i[0,0] = 1
        i[1,1] = 1
        i[2,2] = 1
        w1 = torch.FloatTensor(3,3).fill_(0)
        w1[1, 2] = -1
        w1[2, 1] = 1
        w2 = torch.FloatTensor(3,3).fill_(0)
        w2[2, 0] = -1
        w2[0, 2] = 1
        w3 = torch.FloatTensor(3,3).fill_(0)
        w3[0, 1] = -1
        w3[1, 0] = 1
    ...
    def rays_batch(self, u, v):
        ...
        p = torch.FloatTensor(batch_size, 3,1).fill_(1)
        p[:, 0, 0] = u
        p[:, 1, 0] = v
...
class Mapper():
    ...
    def render_rays(self, u, v, camera, n_coarse=32, n_fine=12, model_freeze=False):
        ...
        with torch.no_grad():
            ds = torch.linspace(0.0001,1.2,n_coarse).to(device).reshape(1, n_coarse).expand(batch_size, n_coarse)
    ...
    def render_small(self, camera, label):
        ...
        with torch.no_grad():
            ...
            depth = torch.FloatTensor(h,w).fill_(0)
            rgb = torch.FloatTensor(h,w,3).fill_(0)
            vs = factor * torch.arange(h).reshape(h,1).expand(h,w).reshape(-1).to(device)
            us = factor * torch.arange(w).reshape(1,w).expand(h,w).reshape(-1).to(device)
            ...
            cv2.imwrite("E:/p2/data/{}_{:04}.png".format(label,self.render_id), prev)
            filepath = "E:/p2/data/{}_{:04}.png".format(label,self.render_id)
        ...
        for camera_id in camera_ids:
            ...
            if activeSampling:
                with torch.no_grad():
                    ...
                    ul=[]
                    vl=[]
                    ri = torch.IntTensor(64).fill_(0)
                    for i in range(64):
                        ni = int(batch_size*self.cameras[camera_id].Li[i])
                        if ni<1:
                            ni=1
                        ri[i] = ni
                        ul.append((torch.rand(ni)*(sw-1)).to(torch.int16).to(device) + (i%8)*sw)
                        vl.append((torch.rand(ni)*(sh-1)).to(torch.int16).to(device) + int(i/8)*sh)
                ...
                us = (torch.rand(batch_size)*(w-1)).to(torch.int16).to(device)
                vs = (torch.rand(batch_size)*(h-1)).to(torch.int16).to(device)
            ...
            if activeSampling:
                with torch.no_grad():
                    ...
                    Li = torch.FloatTensor(64).fill_(0)
